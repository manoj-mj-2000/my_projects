...
<body onload="getData()"> <!-- -->
	...
	<div class="right">
		...
		<form id="cusForm1">
			<table>
				...
				<tr>
					<td>Staff Id</td>
					<td><input type="text" id="stafid"></td>
				</tr>
				<tr>
	...
	var sid, pid, cid, quan, amount, stafid;
	var gaction = "";
	var jsonArr = "";
	...
	function sendData() {
		...
		stafid = document.getElementById("stafid").value;
		var method = "";
		if(gaction == "add"){
			method = "POST";
		}
		...
		if(sid != "" && pid != "" && cid != "" && quan != "" && amount != "" && stafid != ""){
		var dict = {
			"sid" : sid,
			"pid" : pid,
			"cid" : cid,
		...
		xhr.open(method, "http://localhost:8081/vvMart/sales/sale?"
				+ sendstr, true);
		xhr.timeout = 3000;
		xhr.send(sendstr);
		}
	...
	function sendData2() {
		...
		let sendstr = "data=" + uriencode;
		xhr.open("DELETE", "http://localhost:8081/vvMart/sales/sale?"
				+ sendstr, true);
		xhr.timeout = 3000;
		xhr.send(sendstr);
		}
		else{
			alert("Give a valid Sales Id");
		}
	}
	...
	function getData(){
		...
		xhr.open("GET", "http://localhost:8081/vvMart/sales/sale", true);
		xhr.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				gData = this.response;
				setData();
	...
	function setData() {
		...
		for(i in gData){
			...
			td1.innerText = gData[i].sid;
			tr.appendChild(td1);
			td2.innerText = gData[i].pid;
			tr.appendChild(td2);
			td3.innerText = gData[i].cid;
			tr.appendChild(td3);
			td4.innerText = gData[i].quan;
			tr.appendChild(td4);
			td5.innerText = gData[i].amount;
			tr.appendChild(td5);
			td6.innerText = gData[i].stafid;
			tr.appendChild(td6);
	...
	function createtab() {
		...
		th1.innerText = "Sales Id";
		tr.appendChild(th1);
		th2.innerText = "Product Id";
		tr.appendChild(th2);
		th3.innerText = "Customer Id";
		tr.appendChild(th3);
		th4.innerText = "Quantity";
		tr.appendChild(th4);
		th5.innerText = "Amount";
		tr.appendChild(th5);
		th6.innerText = "Staff Id";
		tr.appendChild(th6);
