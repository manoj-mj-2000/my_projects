...
<body onload="getData()">
	...
	<div class="top">
		<p class="adName">Customers Page</p>
	</div>
	...
	<script type="text/javascript">
		...
		function sendData() {
			...
			xhr.open(method, "http://localhost:8081/vvMart/customers/customer?"
					+ sendstr, true);

			xhr.send(sendstr);
			}			
			else{
				alert("Giva valid details to all fields");
			}
		}
		...
		function sendData2() {
			...
			let sendstr = "data=" + uriencode;
			xhr.open("DELETE", "http://localhost:8081/vvMart/customers/customer?"
					+ sendstr, true);

			xhr.send(sendstr);
			}
			else{
				alert("Give valid Customer Id");
			}
		}
		...
		function getData(){
			...
			xhr.open("GET", "http://localhost:8081/vvMart/customers/customer", true);
			xhr.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					gData = this.response;
					setData();
		...
		function createtab() {
			...
			th1.innerText = "Customer Id";
			tr.appendChild(th1);
			th2.innerText = "Customer Name";
			tr.appendChild(th2);
			th3.innerText = "Customer Mobile";
			tr.appendChild(th3);
			th4.innerText = "Customer Email";
			tr.appendChild(th4);
