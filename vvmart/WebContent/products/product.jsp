...
<body onload="getData()">
	...
	<div class="top">
		<p class="adName">Products Page</p>
	</div>
	...
	function sendData() {
		...
		xhr.open(method, "http://localhost:8081/vvMart/products/product?"
				+ sendstr, true);
		xhr.timeout = 3000;
		xhr.send(sendstr);
		}
		else{
			alert("Give valid details to all details");
		}
	...
	function sendData2() {
		...
		let sendstr = "data=" + uriencode;
		xhr.open("DELETE", "http://localhost:8081/vvMart/products/product?"
				+ sendstr, true);
		xhr.timeout = 3000;
		xhr.send(sendstr);
		}
		else{
			alert("Give a valid Product Id");
		}
	}
	...
	function getData(){
		...
		xhr.open("GET", "http://localhost:8081/vvMart/products/product", true);
		xhr.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				gData = this.response;
				setData();
